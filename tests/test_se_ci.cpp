...
// ────────────────────────────────────────────────────────────────────
// Test 9: Asymmetric quadratic — different curvature per dimension
//   f(x,y) = 2*(x-1)^2 + 8*(y-2)^2
//   H = [[4, 0], [0, 16]], SE = [0.5, 0.25]
// ────────────────────────────────────────────────────────────────────
TEST_CASE(se_asymmetric_quadratic) {
    ...
    auto se = fastlckin::compute_standard_errors(func, x_opt);
...
// ────────────────────────────────────────────────────────────────────
// Test 10: SE after optimization — integrate NM + Hessian
// ────────────────────────────────────────────────────────────────────
TEST_CASE(se_after_optimization) {
    ...
    auto result = fastlckin::nelder_mead(quad, {0.0, 0.0}, cfg);
    CHECK(result.converged);
    CHECK_NEAR(result.x[0], 5.0, 0.01);
    CHECK_NEAR(result.x[1], -3.0, 0.01);

    // H = [[2, 0], [0, 8]], SE = [sqrt(0.5), sqrt(0.125)]
    auto se = fastlckin::compute_standard_errors(quad, result.x);
    CHECK(se.size() == 2);
...
// ────────────────────────────────────────────────────────────────────
// Test 11: Step size sensitivity — results should be consistent
// ────────────────────────────────────────────────────────────────────
TEST_CASE(se_step_size_sensitivity) {
    auto quad = [](const std::vector<double>& x) -> double {
    ...
    auto se1 = fastlckin::compute_standard_errors(quad, x_opt, 1e-4);
    auto se2 = fastlckin::compute_standard_errors(quad, x_opt, 1e-5);
    auto se3 = fastlckin::compute_standard_errors(quad, x_opt, 1e-6);
...
// ────────────────────────────────────────────────────────────────────
// Test 12: Analytical SE for asymmetric quadratic
//   f(x,y) = 5*(x-2)^2 + 10*(y-3)^2
//   H = [[10, 0], [0, 20]], SE = [1/sqrt(10), 1/sqrt(20)]
// ────────────────────────────────────────────────────────────────────
...
// ────────────────────────────────────────────────────────────────────
// Test 13: Symmetric Hessian — cross-term function
// ────────────────────────────────────────────────────────────────────
...
TEST_CASE(se_reflects_uncertainty) {
    ...
    auto se_tight = fastlckin::compute_standard_errors(tight_func, x_opt);
    auto se_broad = fastlckin::compute_standard_errors(broad_func, x_opt);
