...
std::vector<SNPInfo> load_bim_snps(const std::string& bim_path) {
    ...
    while (std::getline(ifs, line)) {
        ...
        // We store ref=A2, alt=A1
        // snp_infos[i].af will store the ALT (A1) allele frequency
        s.ref = a2;
        s.alt = a1;
        s.af = 0.0;
        s.af_masked = false;
    ...
    const std::vector<SNPInfo>& snp_infos)
{
    ...
    while (std::getline(ifs, line)) {
        ...
        // Convention: snp_infos[i].af stores the ALT allele frequency,
        // which is freq(A1) in PLINK convention (A1 = ALT).
        //
        // PLINK .frq reports freq(A1) in its MAF column.
        if (si.alt == a1 && si.ref == a2) {
            // Standard: .frq A1 = our ALT (= PLINK A1)
            // freq(A1) = maf = freq(ALT) → direct
            freqs[idx] = maf;
        } else if (si.alt == a2 && si.ref == a1) {
            // Swapped: .frq A1 = our REF (= PLINK A2)
            // freq(A1) = maf = freq(REF) → freq(ALT) = 1 - maf
            freqs[idx] = 1.0 - maf;
        } else {
    ...
    const std::vector<int>& snp_indices)
{
    ...
    for (int s = 0; s < n_snps; ++s) {
        ...
        for (int i = 0; i < n_samples; ++i) {
            ...
            // g counts A1 copies: 0→0, 1→1, 2→2
            // Returns freq(A1) = ALT frequency, consistent with EM and
            // read_plink_freq().
            allele_count += g;
            valid_count += 2;
        }
