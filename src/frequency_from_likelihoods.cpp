    ...
    bool verbose)
{
    ...
    std::vector<bool> exclude(n_samples, false);
    ...
    for (int s = 0; s < n_snps; ++s) {
        ...
        for (int iter = 0; iter < max_iter; ++iter) {
            ...
            for (int i = 0; i < n_samples; ++i) {
                // Skip excluded samples (leave-one-out pair)
    ...
    if (verbose) {
        std::cerr << "[fastlckin] Leave-one-out EM AF estimation: " << n_snps
                  << " SNPs, excluded " << exclude_indices.size() << " samples\n";
    }
