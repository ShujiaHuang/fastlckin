...
std::vector<double> compute_standard_errors(
    ...
    double h)
{
    int n = static_cast<int>(x_opt.size());
    
    // Compute Hessian matrix using finite differences
    // H[i][j] = (f(x+h_i+h_j) - f(x+h_i-h_j) - f(x-h_i+h_j) + f(x-h_i-h_j)) / (4*h^2)
    std::vector<std::vector<double>> H(n, std::vector<double>(n, 0.0));
    
    double f0 = func(x_opt);
    
    for (int i = 0; i < n; ++i) {
        for (int j = i; j < n; ++j) {
            std::vector<double> x_pp = x_opt;
            std::vector<double> x_pm = x_opt;
            std::vector<double> x_mp = x_opt;
            std::vector<double> x_mm = x_opt;
            ...
            double f_pp = func(x_pp);
            double f_pm = func(x_pm);
            double f_mp = func(x_mp);
            double f_mm = func(x_mm);
    ...
    // For 2D case, invert Hessian analytically
    // H^{-1} = (1/det) * [H[1][1], -H[0][1]; -H[1][0], H[0][0]]
    if (n == 2) {
        double det = H[0][0] * H[1][1] - H[0][1] * H[1][0];
        
        if (std::abs(det) < 1e-15) {
            // Hessian is singular, return empty SEs
            return {};
        }
        
        // Covariance matrix = H^{-1}
        double var0 = H[1][1] / det;
        double var1 = H[0][0] / det;
        
        // Standard errors = sqrt(diagonal of covariance)
        // Only return if variances are positive
        if (var0 <= 0 || var1 <= 0) {
            return {};
        }
        
        return {std::sqrt(var0), std::sqrt(var1)};
    }
    
    // For general n-dimensional case, would need full matrix inversion
    // For now, return empty for n != 2
    return {};
}
